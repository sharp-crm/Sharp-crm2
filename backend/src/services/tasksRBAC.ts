...
export interface Task {
  ...
  status: string;
  dueDate: string;
  assignee: string; // Frontend field name
  assignedTo?: string; // Backend field name
...
export class TasksRBACService {
   ...
   */
  async getTaskByIdForUser(taskId: string, user: RBACUser): Promise<Task | null> {
  ...
  async getTasksByRelatedRecordForUser(recordType: string, recordId: string, user: RBACUser): Promise<Task[]> {
    ...
    const relatedTasks = accessibleTasks.filter(task => 
      task.relatedRecordType === recordType && task.relatedRecordId === recordId
    );
   ...
   */
  private async canUserAccessTask(task: Task, user: RBACUser): Promise<boolean> {
   ...
   */
  private async canUserAccessTasksFromAssignee(assigneeId: string, user: RBACUser): Promise<boolean> {
    switch (user.role) {
      case 'ADMIN':
   ...
   */
  private buildTenantFilter(tenantId: string, includeDeleted: boolean): TaskAccessFilter {
    return {
   ...
   */
  private async buildRoleBasedFilter(user: RBACUser): Promise<TaskAccessFilter> {
    switch (user.role) {
      case 'ADMIN':
   ...
   */
  private combineFilters(tenantFilter: TaskAccessFilter, roleFilter: TaskAccessFilter): TaskAccessFilter {
...
export const tasksRBACService = new TasksRBACService();
