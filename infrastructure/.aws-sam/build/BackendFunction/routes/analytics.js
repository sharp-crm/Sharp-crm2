...
router.get("/overview", async (req, res, next) => {
    try {
        ...
        const [dealsResult, leadsResult, tasksResult, contactsResult] = await Promise.all([
            dynamoClient_1.docClient.send(new lib_dynamodb_1.ScanCommand({ TableName: dynamoClient_1.TABLES.DEALS })),
            dynamoClient_1.docClient.send(new lib_dynamodb_1.ScanCommand({ TableName: dynamoClient_1.TABLES.LEADS })),
            dynamoClient_1.docClient.send(new lib_dynamodb_1.ScanCommand({ TableName: dynamoClient_1.TABLES.TASKS })),
            dynamoClient_1.docClient.send(new lib_dynamodb_1.ScanCommand({ TableName: dynamoClient_1.TABLES.CONTACTS }))
        ]);
...
router.get("/leads", async (req, res, next) => {
    try {
        const result = await dynamoClient_1.docClient.send(new lib_dynamodb_1.ScanCommand({ TableName: dynamoClient_1.TABLES.LEADS }));
...
router.get("/deals", async (req, res, next) => {
    try {
        const result = await dynamoClient_1.docClient.send(new lib_dynamodb_1.ScanCommand({ TableName: dynamoClient_1.TABLES.DEALS }));
    ...
    try {
        ...
        const recentActivity = {
            ...
            completedTasks: tasks.filter(t => t.status === 'Completed' &&
                new Date(t.createdAt) > thirtyDaysAgo).length
        };
        res.json({
            data: {
                taskStats,
...
router.get("/team", async (req, res, next) => {
    try {
        const [dealsResult, tasksResult, usersResult] = await Promise.all([
            dynamoClient_1.docClient.send(new lib_dynamodb_1.ScanCommand({ TableName: dynamoClient_1.TABLES.DEALS })),
            dynamoClient_1.docClient.send(new lib_dynamodb_1.ScanCommand({ TableName: dynamoClient_1.TABLES.TASKS })),
            dynamoClient_1.docClient.send(new lib_dynamodb_1.ScanCommand({ TableName: "Users" }))
        ]);
        const deals = dealsResult.Items || [];
        const tasks = tasksResult.Items || [];
