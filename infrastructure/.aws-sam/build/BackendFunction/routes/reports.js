...
router.get("/", async (req, res, next) => {
    try {
        ...
        const result = await dynamoClient_1.docClient.send(new lib_dynamodb_1.ScanCommand({
            TableName: dynamoClient_1.TABLES.REPORTS,
        ...
        let reports = (result.Items || []).filter(report => {
            ...
            if (report.sharedWith && Array.isArray(report.sharedWith) && report.sharedWith.includes(userId))
                return true;
            return false;
        });
...
router.post("/", async (req, res, next) => {
    try {
        const { name, description, reportType, // 'sales', 'leads', 'contacts', 'deals', 'tasks', 'custom'
        filters, columns, chartType, schedule, isPublic = false } = req.body;
        if (!name || !reportType) {
            throw (0, errorHandler_1.createError)("Name and report type are required", 400);
        }
...
router.post("/:id/run", async (req, res, next) => {
    try {
        ...
        const reportResult = await dynamoClient_1.docClient.send(new lib_dynamodb_1.GetCommand({
            TableName: dynamoClient_1.TABLES.REPORTS,
            Key: { id }
        }));
...
async function generateReportData(reportType, tenantId, userId) {
    switch (reportType) {
        case 'deals-pipeline': {
            const deals = await deals_1.dealsService.getDealsByTenant(tenantId, userId);
        ...
        case 'sales-forecast': {
            ...
            }, { totalPipeline: 0, weightedPipeline: 0, byProbability: {} });
            return {
                forecast,
                timeline: {
        ...
        case 'lead-conversion': {
            const leads = await dynamoClient_1.docClient.send(new lib_dynamodb_1.ScanCommand({
                TableName: dynamoClient_1.TABLES.LEADS,
                FilterExpression: 'tenantId = :tenantId',
                ExpressionAttributeValues: {
                    ':tenantId': tenantId
                }
            }));
            const leadData = (leads.Items || []).reduce((acc, lead) => {
            ...
            return {
                ...
                summary: {
                    ...
                    convertedLeads: (leads.Items || []).filter((l) => l.status === 'Converted').length,
                    averageValue: (leads.Items || [])
                        .filter((l) => l.status === 'Converted')
                        .reduce((sum, l) => sum + (l.value || 0), 0) /
                        (leads.Items || []).filter((l) => l.status === 'Converted').length || 0
                }
            };
        }
        case 'task-completion': {
            const tasks = await dynamoClient_1.docClient.send(new lib_dynamodb_1.ScanCommand({
            ...
            const taskData = (tasks.Items || []).reduce((acc, task) => {
            ...
            return {
                ...
                summary: {
                    ...
                    overdueTasks: (tasks.Items || []).filter((t) => new Date(t.dueDate) < new Date() && t.status !== 'Completed').length
                }
            };
        }
        case 'contact-engagement': {
            // Use QueryCommand with TenantIndex GSI for better performance instead of ScanCommand
            const contacts = await dynamoClient_1.docClient.send(new lib_dynamodb_1.QueryCommand({
                TableName: dynamoClient_1.TABLES.CONTACTS,
            ...
            const contactData = (contacts.Items || []).reduce((acc, contact) => {
                ...
                acc[source].total++;
                const contactTasks = (tasks.Items || []).filter((t) => t.contactId === contact.id);
            ...
            return {
                ...
                summary: {
                    ...
                    activeContacts: (contacts.Items || []).filter((c) => (tasks.Items || []).some((t) => t.contactId === c.id)).length,
                    totalInteractions: (tasks.Items || []).filter((t) => t.contactId).length
                }
            };
        }
        default:
            throw (0, errorHandler_1.createError)("Unsupported report type", 400);
    }
}
// Update report
router.put("/:id", async (req, res, next) => {
    try {
        ...
        if (getResult.Item.createdBy !== userId) {
            throw (0, errorHandler_1.createError)("Access denied. Only report creator can update.", 403);
        }
...
router.delete("/:id", async (req, res, next) => {
    try {
        ...
        if (getResult.Item.createdBy !== userId) {
            throw (0, errorHandler_1.createError)("Access denied. Only report creator can delete.", 403);
        }
...
router.post("/:id/share", async (req, res, next) => {
    try {
        ...
        if (getResult.Item.createdBy !== userId) {
            throw (0, errorHandler_1.createError)("Access denied. Only report creator can share.", 403);
        }
        // Validate userIds array
        if (userIds && Array.isArray(userIds)) {
            // Verify all userIds exist in the tenant
            const userResults = await Promise.all(userIds.map(uid => dynamoClient_1.docClient.send(new lib_dynamodb_1.ScanCommand({
            ...
            if (invalidUsers.length > 0) {
                throw (0, errorHandler_1.createError)(`Invalid user IDs: ${invalidUsers.join(", ")}`, 400);
            }
        }
        ...
        if (updateExpression.length === 0) {
            throw (0, errorHandler_1.createError)("No sharing settings provided", 400);
        }
        const result = await dynamoClient_1.docClient.send(new lib_dynamodb_1.UpdateCommand({
            TableName: dynamoClient_1.TABLES.REPORTS,
            Key: { id },
