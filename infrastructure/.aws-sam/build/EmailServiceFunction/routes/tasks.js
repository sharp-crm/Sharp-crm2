...
const getAllTasks = async (req, res) => {
    ...
    try {
        ...
        if (recordType && recordId) {
            ...
            tasks = await tasks_1.tasksService.getTasksByRelatedRecordForUser(recordType, recordId, rbacUser);
        }
        else {
            tasks = await tasks_1.tasksService.getTasksForUser(rbacUser, includeDeleted);
        }
...
const getTaskById = async (req, res) => {
    ...
    try {
        ...
        const task = await tasks_1.tasksService.getTaskByIdForUser(id, rbacUser);
        if (!task) {
...
const getTasksByAssignee = async (req, res) => {
    ...
    try {
        ...
        const tasks = await tasks_1.tasksService.getTasksByAssigneeForUser(assignee, rbacUser);
...
const getTasksByStatus = async (req, res) => {
    ...
    try {
        ...
        const tasks = await tasks_1.tasksService.getTasksByStatusForUser(status, rbacUser);
...
const searchTasks = async (req, res) => {
    ...
    try {
        ...
        const tasks = await tasks_1.tasksService.searchTasksForUser(rbacUser, q);
...
const getTasksStats = async (req, res) => {
    ...
    try {
        ...
        const stats = await tasks_1.tasksService.getTasksStatsForUser(rbacUser);
...
const createTask = async (req, res) => {
    ...
    try {
        ...
        const task = await tasks_1.tasksService.createTask(taskInput, user.userId, user.email, user.tenantId);
...
const updateTask = async (req, res) => {
    ...
    try {
        ...
        const task = await tasks_1.tasksService.updateTask(id, updateInput, user.userId, user.email, user.tenantId);
        if (!task) {
...
const deleteTask = async (req, res) => {
    ...
    try {
        ...
        const success = await tasks_1.tasksService.deleteTask(id, user.userId, user.email, user.tenantId);
        if (!success) {
            res.status(404).json({ message: "Task not found" });
            return;
        }
...
const hardDeleteTask = async (req, res) => {
    ...
    try {
        ...
        const success = await tasks_1.tasksService.hardDeleteTask(id, user.tenantId);
        if (!success) {
            res.status(404).json({ message: "Task not found" });
            return;
        }
